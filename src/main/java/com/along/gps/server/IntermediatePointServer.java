...
import com.along.gps.pojo.GpsDescData;
...
public class IntermediatePointServer{
	...
	public static void gpsData(GpsDescData gdd){
		if(GPS_DATA.get(gdd.getCrossGpsData().getTaskId()+"")!=null){//判断该任务是否存在
			if (GPS_DATA.get(gdd.getCrossGpsData().getTaskId()+"").get(gdd.getCrossGpsData().getEquipmentId()+"")!=null){//判断该任务中是否存在该设备
				GPS_DATA.get(gdd.getCrossGpsData().getTaskId()+"").get(gdd.getCrossGpsData().getEquipmentId()+"").add(gdd);
			}else{
				List<GpsDescData> list= new ArrayList<>();
				list.add(gdd);
				GPS_DATA.get(gdd.getCrossGpsData().getTaskId()+"").put(gdd.getCrossGpsData().getEquipmentId()+"",list);
			}
		}else{
			Map<String,List<GpsDescData>> DATA=new HashMap<>();
			List<GpsDescData> list= new ArrayList<>();
			list.add(gdd);
			DATA.put(gdd.getCrossGpsData().getEquipmentId()+"",list);
			GPS_DATA.put(gdd.getCrossGpsData().getTaskId()+"",DATA);
		}
	}
	...
	public static void main(String args[]) {
		...
		new IntermediatePointServer().listen();
	}
							...
							ch.pipeline().addLast(new SimpleChannelInboundHandler<ByteBuf>() {
								...
								protected void channelRead0(ChannelHandlerContext ctx, ByteBuf bf) throws Exception {
									...
									try {
										...
										String data2 = ConvertData.getHexMsgToString(hexStr);
										System.out.println("json:"+data2);
										//saveMsg(ctx, hexStr);
										//数据处理
								...
								public void channelActive(ChannelHandlerContext ctx) throws Exception {
									System.out.println(ctx.channel().remoteAddress() + "->tcp连接成功：channelActive");
								}
